...
exports.updateMeetup = exports.getMeetups = exports.getMeetupById = exports.deleteMeetup = exports.createMeetup = void 0;
...
var getMeetups = function getMeetups(req, res) {
  ...
  (0, _queries.selectAll)(req.query).then(function (_ref) {
    var rows = _ref.rows;
...
var getMeetupById = function getMeetupById(req, res) {
  ...
  (0, _queries.selectById)({
    ...
    if (data.rowCount === 0) return res.status(404).json('Data with such id was not found');
...
var createMeetup = function createMeetup(req, res) {
  ...
  if (validationError) throw res.status(400).json(validationError.message);
  (0, _queries.insertData)(req.body).then(function (data) {
    return res.status(201).send("Meetup added with ID: ".concat(data.rows[0].id));
  })["catch"](function (err) {
    throw err;
  });
};
...
var updateMeetup = function updateMeetup(req, res) {
  var id = parseInt(req.params.id);
  var validationError = _schema.bodyParams.validate(req.body).error || _schema.idParam.validate(id).error;
  if (validationError) throw res.status(400).json(validationError.message);
  (0, _queries.selectById)({
    id: id
  }).then(function (data) {
    if (data.rowCount === 0) return res.status(404).json('Data with such id was not found');
    (0, _queries.updateById)(_objectSpread(_objectSpread({}, req.body), {}, {
      id: id
    })).then(function () {
